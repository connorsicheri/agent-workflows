...
name: visual-recap
description: >-
  Deprecated parked Compass skill. Kept for future local visual recap work, but
...
# Visual Recap Deprecated

This skill is currently deprecated in Compass. It is kept in the repository as
parked source material for a possible future local recap viewer, but active
...
## Compass Local Recap Artifacts

In Compass, visual recaps are local repo artifacts by default. Write the recap
under the repository root `plans/<slug>/`; do not require the hosted Plan MCP
connector, and do not publish to the hosted Plan app unless the user explicitly
asks for hosted publishing or commenting.

The local-files contract is:

- Read the diff/stat/source context from local files and shell commands only.
- Do not fetch a live block catalog, run a local bridge, or use external Plan
  validation unless the user explicitly asks for that extra validation or
  rendered preview.
...
- Write the recap as a local MDX folder under `plans/<slug>/`. The folder
  contains `plan.mdx`, optional `canvas.mdx`, optional `prototype.mdx`, and
  optional `.plan-state.json`. Set `kind: "recap"` and `localOnly: true` in
...
- Treat review feedback as file or chat feedback: update the MDX files directly,
  and summarize the changed local paths. Hosted comments, sharing, screenshots,
  usage attachment, and PR sticky comment publishing are unavailable until the
...
## Create A Structured Local Recap — Never Inline

The deliverable is ALWAYS a structured local Agent-Native recap folder, not a
chat-only recap. NEVER hand the recap to the user as inline chat content — not
Markdown prose, not an ASCII sketch, not a table, not a fenced "wireframe", not
a "here's the recap" summary. A short chat handoff may summarize the local path
and review focus, but the recap itself belongs in `plans/<slug>/`.
...
validation unless the user explicitly asks for that extra validation or rendered
...
- Use a `Before` / `After` wireframe pair when the reviewer benefits from direct
  ...
  Use columns for narrow focused surfaces and vertical stacking for wide desktop
  or browser frames.
...
When a recap includes a top canvas, storyboard, or flow view, author
`canvas.mdx` directly. Recap canvas artboards must use the same HTML wireframe
path as good document-body wireframes: `<Screen surface="..." html={...} />`
with a semantic HTML fragment. Do not author fresh kit-tree children such as
`<FrameScreen>`, `<Card>`, `<Row>`, `<Title>`, or `<Btn>` inside canvas
`<Screen>` tags. Those components are legacy compatibility markup for old plans;
in new canvas storyboards they can produce cramped or overlapping layouts even
when the inline body wireframe looks good. If a canvas mockup looks worse than
the same screen below the fold, assume it used the legacy kit path and replace it
with an HTML screen.

## Open And Report The Recap

Report the local artifact path, usually `plans/<slug>/plan.mdx`. If the user
explicitly requested a rendered preview and approved the optional command, also
report the preview URL. Do not invent a hosted database URL and do not publish
just to get an absolute Plan link. Do not fall back to an inline recap.

When the user explicitly opts into hosted publishing, use the hosted Plan tool's
returned absolute URL and make access expectations clear for private repos.

## Diff → Block Mapping

Map each kind of change to the block that carries it, derived mechanically from
the actual diff. The names below are the conceptual block types to use in the
local MDX recap. Use the bundled references for exact authoring patterns.
...
- **Architecture or data-flow shift** → `diagram` with `data.html` / `data.css`
  ...
  the same `--wf-*` theme tokens used by wireframes — never
...
## Block Reference — Use Bundled References
...
Before writing structured recap content, read the bundled visual-plan and
visual-recap references. Do not fetch a live block catalog by default. If the
user explicitly asks for rendered validation/preview, you may use optional Plan
...
- `WireframeBlock`: its body is a single `<Screen surface ... html=… />` subtree
  ...
  template literal, never a dynamic `html={someVar}` expression.
...
so the reviewer can trace the actual user path. Use columns for narrow focused
surfaces and vertical stacking for wide `desktop`/`browser` frames; never
...
- **Gate visibility.** Local recap files follow repository access rules. If the
  user explicitly asks to publish a hosted recap for a private repo, set the
...
Because a recap is a real, editable local plan, review feedback should update
the files under `plans/<slug>/` directly. Treat comments as chat or file review
feedback, then patch `plan.mdx`, `canvas.mdx`, or `prototype.mdx`. Hosted
`get-plan-feedback`, `create-visual-recap`, and `update-visual-plan` belong only
to the optional hosted publishing path.
