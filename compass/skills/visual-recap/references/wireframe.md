...
**Editing an existing mockup.** In Compass local-files mode, edit the relevant
`plans/<slug>/plan.mdx` or `canvas.mdx` HTML snippet directly. External
validation is optional and should only run when the user asks for it.
